...
def none_zero_value(f):
  '''
  Read a sim file and display whether each line contains a non-zero value 
  other than the last three, which are atoms in a water molecule.    
  '''
  lines = f.readlines()
  for line in lines:
    densities = line.split()
...
def main():
  ...
  if len(sys.argv) == 1:
    print("Enter a filename. The syntax:\n",
          "python SIMtoEXP_utility filename.sim")
    return
  elif len(sys.argv) == 2:
    filename = sys.argv[1]
  else:
